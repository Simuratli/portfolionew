...
import { ref as storageRefElem, uploadBytesResumable, getDownloadURL } from 'firebase/storage';
    ...
    <div className="without_avatar">
      ...
      <Toggle header="Musics in Database">
        <div className={classes.musiclist}>
          {
            data.map((music) => {
              return <div key={music?.url} className={classes.musiclistdb}>
                <p>{music?.name}</p>
                <AiOutlineDelete />
              </div>
            })
      ...
      {
        loading && <div className={classes.musiclist__loader}>
          <div>
            <Image
              src={image ? image : 'https://images.unsplash.com/photo-1590615370581-265ae19a053b?ixlib=rb-4.0.3&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=1470&q=80'}
              width={200}
              height={210}
            />
          </div>
      ...
      <Input onChange={(e) => { setImage(e.target.value) }} type="text" placeholder="Image of music" value={image} label={"Image"} />
