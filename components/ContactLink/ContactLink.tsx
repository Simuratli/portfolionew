import React from 'react';
import classes from '../../styles/components/contactlink.module.scss';
import { ContactLinkPropTypes } from './ContactLinkTypes';

function ContactLink({ icon, text, link }: ContactLinkPropTypes) {
  return (
    <a target='_blank' href={link} className={classes.contactLink} rel='noreferrer'>
      <span>{icon}</span>
      <svg
        width='20'
        height='20'
        viewBox='0 0 20 20'
        fill='none'
        xmlns='http://www.w3.org/2000/svg'
      >
        <path
          d='M14.9333 5.51667C14.8487 5.31304 14.6869 5.15123 14.4833 5.06667C14.3831 5.02397 14.2755 5.00132 14.1666 5H5.83328C5.61227 5 5.40031 5.0878 5.24403 5.24408C5.08775 5.40036 4.99995 5.61232 4.99995 5.83333C4.99995 6.05435 5.08775 6.26631 5.24403 6.42259C5.40031 6.57887 5.61227 6.66667 5.83328 6.66667H12.1583L5.24162 13.575C5.16351 13.6525 5.10151 13.7446 5.05921 13.8462C5.0169 13.9477 4.99512 14.0567 4.99512 14.1667C4.99512 14.2767 5.0169 14.3856 5.05921 14.4871C5.10151 14.5887 5.16351 14.6809 5.24162 14.7583C5.31908 14.8364 5.41125 14.8984 5.5128 14.9407C5.61435 14.9831 5.72327 15.0048 5.83328 15.0048C5.94329 15.0048 6.05221 14.9831 6.15376 14.9407C6.25531 14.8984 6.34748 14.8364 6.42495 14.7583L13.3333 7.84167V14.1667C13.3333 14.3877 13.4211 14.5996 13.5774 14.7559C13.7336 14.9122 13.9456 15 14.1666 15C14.3876 15 14.5996 14.9122 14.7559 14.7559C14.9122 14.5996 14.9999 14.3877 14.9999 14.1667V5.83333C14.9986 5.72444 14.976 5.61685 14.9333 5.51667V5.51667Z'
          fill='#37352F'
        />
      </svg>
      <span className={classes.contactLink__text}>{text}</span>
    </a>
  );
}

export default ContactLink;
